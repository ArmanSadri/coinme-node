...
let converter = new Converter({
    ...
    conversions: {
         ...
         * @returns {Number}
         */
        'Bitcoin->Bitcoin': function(valueInBitcoin) {
            Preconditions.shouldBeNumber(valueInBitcoin);

            return valueInBitcoin;
        },
         ...
         * @returns {Number}
         */
        'Satoshi->Satoshi': function(valueInSatoshi) {
            Preconditions.shouldBeNumber(valueInSatoshi);

            return valueInSatoshi;
        }
    }
});
...
export default class Bitcoin extends Currency {
     ...
     * @returns {Function}
     */
    static converterTo(currency) {
        currency = Currency.getCurrency(currency);

        return Currency.converter(Bitcoin, currency);
    }
